...
function injectTransitionProgress({
	definition,
	passes,
	progress,
}: {
	...
	return passes.map((pass) => ({
		...pass,
		uniforms: { ...pass.uniforms, u_progress: p },
	}));
}
...
function resolveEffectPassGroups({
	...
	return (effects ?? [])
		...
		.map((effect) => {
			...
			return injectTransitionProgress({
				definition,
				passes,
...
function resolveEffectLayerNode({
	...
	const passes = injectTransitionProgress({
		definition,
		passes: resolveEffectPasses({
			definition,
		...
		progress:
			node.params.duration > 0 ? localTime / node.params.duration : 0,
	});
	if (passes.length === 0) {
		return null;
	}
