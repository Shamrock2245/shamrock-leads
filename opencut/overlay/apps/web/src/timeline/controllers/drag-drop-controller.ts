...
export class DragDropController {
	...
	private executeAssetDrop({
		...
		switch (dragData.type) {
			...
			case "effect":
				...
				return;
			case "transition":
				this.executeTransitionDrop({ dragData });
				return;
			case "media":
	...
	private executeTextDrop({
		...
		const preset = TEXT_PRESETS.find((item) => item.id === dragData.id);
		const element = preset
			? buildPresetTextElement({
					preset,
	...
	private executeTransitionDrop({
		...
		applyTransitionAtPlayhead({ type: dragData.transitionType });
	}
