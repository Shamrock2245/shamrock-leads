...
function SoundEffectsView() {
	const [library, setLibrary] = useState<"bundled" | "freesound">("bundled");
	const {
		topSoundEffects,
		isLoading,
		searchQuery,
	...
	const {
		...
		query: library === "freesound" ? searchQuery : "",
		...
		const loadBundled = async () => {
			try {
				...
				const q = searchQuery.trim()
					...
					: "";
				const response = await fetch(`/api/media/catalog${q}`);
				...
				setTotalCount({ count: data.count || 0 });
			} catch (error) {
				if (!ignore) {
					setError({
						error:
							error instanceof Error ? error.message : "Failed to load bundled SFX",
					});
				}
			} finally {
				if (!ignore) setLoading({ loading: false });
			}
		};
		const t = setTimeout(loadBundled, 80);
		return () => {
			ignore = true;
			clearTimeout(t);
		};
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center gap-2">
				<Button
					variant={library === "bundled" ? "default" : "text"}
					size="sm"
					onClick={() => {
						setLibrary("bundled");
				...
				<Button
					variant={library === "freesound" ? "default" : "text"}
					size="sm"
					onClick={() => {
						setLibrary("freesound");
						setHasLoaded({ loaded: false });
					}}
				>
					Freesound
