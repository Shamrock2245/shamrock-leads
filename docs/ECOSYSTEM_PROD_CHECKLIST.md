...
| P1.8 | Gmail Pub/Sub authenticated push: dedicated service account and exact `GMAIL_PUBSUB_AUDIENCE`, `GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL`, `GMAIL_PUBSUB_SUBSCRIPTION`, `GMAIL_MONITORED_MAILBOX` values set on VPS | leads | [ ] **Human-gated open** — Endpoint code & tests complete; live probe returned HTTP 503 fail-closed (VPS environment variables unset); GCP Pub/Sub push subscription setup required. |
