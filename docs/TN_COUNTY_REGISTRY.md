...
| Giles | `giles.py` | 120 min | Southern Software | **Locally validated fail-closed safeguard pending deployment.** Configured Citizen Connect agency `GilesCoTN` resolves to the generic agency directory rather than a Giles booking roster. The county guard emits no records and does not invoke the shared parser until an official broad roster supplies a source-issued booking/inmate ID and booking time. |
