...
| Hamblen | `hamblen.py` | 90 min | Zuercher | **Locally validated fail-closed safeguard pending deployment.** The configured Zuercher hostname no longer resolves normally, and the currently accessible official ISOMS listing exposes intake timing but no verified source-issued booking/inmate identity contract. The county guard emits no records until a compliant broad roster is validated. |
