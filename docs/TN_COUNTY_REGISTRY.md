...
| Robertson | `robertson.py` | 90 min | Southern Software | **Locally validated fail-closed safeguard pending deployment.** Configured Citizen Connect agency `RobertsonCoTN` resolves to the generic agency directory rather than a Robertson booking roster. The county guard emits no records and does not invoke the shared parser until an official broad roster supplies a source-issued booking/inmate ID and booking time. |
