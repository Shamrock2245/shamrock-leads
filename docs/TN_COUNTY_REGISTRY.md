...
| Coffee | `coffee.py` | 120 min | Southern Software | **Locally validated fail-closed safeguard pending deployment.** Configured Citizen Connect agency `CoffeeCoTN` resolves to the generic agency directory rather than a Coffee booking roster. The county guard emits no records and does not invoke the shared parser until an official broad roster supplies a source-issued booking/inmate ID and booking time. |
