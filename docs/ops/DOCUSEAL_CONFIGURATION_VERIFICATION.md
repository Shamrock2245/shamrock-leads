...
> **Correction (2026-09-25):** Palmetto is **template `5`** (`DOCUSEAL_TEMPLATE_ID_PALMETTO=5`, per `.env.example` and `docs/PAPERWORK_PORTAL_DOCUSEAL.md`, changed 2026-09-22 / PR #42), **not** `3`. The live-probe details in §3 below are the historical 2026-08-22 record (when ID `3` was Palmetto and ID `5` was listed as a backup). Re-verify names/roles/fields with the read-only drift guard: `python scripts/docuseal_template_drift_check.py` (see `tests/fixtures/docuseal_template_snapshot.json`).
...
| `DOCUSEAL_TEMPLATE_ID_PALMETTO` | Palmetto Surety Corporation Packet | `shamrock-palmetto-paperwork-complete` | `5` | HTTPS GET `/api/templates/5` | `indemnitor`, `defendant`, `bondsman`, `coindemnitor` | ⚠️ Re-verify (ID changed 3 → 5 on 2026-09-22; see note below) |
...
- **Palmetto Fallback Guard:** Verified that `resolve_template_id_for_surety("palmetto")` strictly reads `DOCUSEAL_TEMPLATE_ID_PALMETTO` (ID `5`; was `3` at the time of this 2026-08-22 audit). It **never** silently falls back to OSI when Palmetto's template is absent or invalid (`tests/test_docuseal_surety_template_guards.py::test_palmetto_never_falls_back_to_osi` ✅).
...
### Probe 2: Authenticated Template Directory (historical, 2026-08-22 — Palmetto is now ID `5`)
...
- **B5 (`DOCUSEAL_*` valid / templates resolve):** 🟢 Verified Live on 2026-08-22 for templates 1 & 3 (historical). Palmetto has since moved to template **5** — re-verify 1 & 5 with `scripts/docuseal_template_drift_check.py`.
