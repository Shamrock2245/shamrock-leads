...
| Forrest | `forrest.py` | 120 min | **Fail-closed safeguard deployed 2026-08-15.** The configured official API returned HTTP 404 and the prior parser used an unsafe `id` fallback without a verified booking timestamp. The registered guard emits no records until a source-issued booking-safe API contract is revalidated. Deployment run `31855583732` completed successfully; Forrest-specific persistence and alert telemetry remain unproven. |
| Harrison | `harrison.py` | 120 min | **Fail-closed safeguard deployed 2026-08-15.** The configured API returned HTTP 200 but no parseable public field contract, while the prior parser used an unsafe `id` fallback without a verified booking timestamp. The registered guard emits no records until a source-issued booking-safe API contract is revalidated. Deployment run `31855845579` completed successfully; Harrison-specific persistence and alert telemetry remain unproven. |
...
| Jackson | `jackson.py` | 120 min | **Fail-closed safeguard deployed 2026-08-15.** Direct normal access to the configured official list returned HTTP 403 with access-control markers. The prior path used a residential-proxy stealth session, CAPTCHA bypass behavior, profile routes, DOB, and synthetic IDs. The registered guard emits no records until a normal-access booking-safe contract is revalidated. Deployment run `31856142991` completed successfully; Jackson-specific persistence and alert telemetry remain unproven. |
| Jones | `jones.py` | 120 min | **Inherited JailTracker fail-closed safeguard deployed.** Revalidated 2026-08-15: official landing page requires image-character human verification. The shared guard makes no CAPTCHA attempt, proxy request, profile request, sensitive-field collection, or synthetic key; it emits no records until a separately verified listing-only contract exists. |
| Lauderdale | `lauderdale.py` | 120 min | **Inherited JailTracker fail-closed safeguard deployed.** Revalidated 2026-08-15: official landing page requires image-character human verification. The shared guard makes no CAPTCHA attempt, proxy request, profile request, sensitive-field collection, or synthetic key; it emits no records until a separately verified listing-only contract exists. |
