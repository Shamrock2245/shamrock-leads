...
| Jackson | `jackson.py` | 120 min | **Locally validated fail-closed safeguard pending deployment.** Direct normal access to the configured official list returned HTTP 403 with access-control markers. The prior path used a residential-proxy stealth session, CAPTCHA bypass behavior, profile routes, DOB, and synthetic IDs. The registered guard emits no records until a normal-access booking-safe contract is revalidated. |
