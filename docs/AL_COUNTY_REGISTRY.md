...
| Lee | `lee.py` | 120 min | **Local two-page official-source smoke passed 2026-08-14**; Mongo upsert and alert delivery remain unproven. |
...
The bounded local smoke over the first two official pages parsed 20 records with Alabama/Lee state invariants, non-empty dedup keys, and source-key provenance. This is local source validation only, not evidence of a production database write or notification.
