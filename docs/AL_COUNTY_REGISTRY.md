...
| Jefferson | `jefferson.py` | 120 min | **Locally validated fail-closed safeguard pending deployment.** Direct normal access to the configured official New World portal returned HTTP 403. The prior path declared a residential-proxy stealth session; the county guard now emits no records until an official broad roster supplies a source-issued booking/inmate ID and booking time without bypassing controls. |
