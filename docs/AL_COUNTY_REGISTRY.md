...
| Houston | `houston.py` | 120 min | **Locally validated fail-closed safeguard pending deployment.** Direct normal access to configured Citizen Connect agency `HoustonCoAL` returned HTTP 403 and no listing cards or source-issued booking identity contract. The county guard emits no records and does not invoke the shared parser until an official broad roster supplies a source-issued booking/inmate ID and booking time. |
