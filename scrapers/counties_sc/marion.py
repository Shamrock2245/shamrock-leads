"""
Marion County (SC) Arrest Scraper.
Portal: https://jail.marionso.com/
Often blocked (403/Cloudflare) from datacenter IPs. Attempts HTTP fetch;
set MARION_SOCKS_PROXY or SOCKS_PROXY for residential egress.
"""
...
PORTAL_URL = "https://jail.marionso.com/"
    ...
    def scrape(self) -> List[ArrestRecord]:
        ...
        try:
            ...
            if resp.status_code in (403, 503):
                logger.warning(
                    f"Marion: HTTP {resp.status_code} — bot protection. "
                    f"Set SOCKS_PROXY for residential IP."
                )
                return []
            resp.raise_for_status()
            records = self._parse(resp.text)
            logger.info(f"Marion: {len(records)} records in {time.time()-start:.1f}s")
            return records
