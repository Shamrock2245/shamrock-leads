...
Portal: https://hamptonjailroster.org/
Often blocked (403/Cloudflare) from datacenter IPs. Attempts HTTP fetch;
set HAMPTON_SOCKS_PROXY or SOCKS_PROXY for residential egress.
"""
...
PORTAL_URL = "https://hamptonjailroster.org/"


class HamptonScraper(BaseScraper):
    # Hold (docs/recon/SC_WRITE_SMOKE_2026-09-24.md): hamptonjailroster.org answers HTTP 403 /
    ...
    SOURCE_CONTRACT_REASON = (
        "Hampton (SC) hold: portal returns 403/Cloudflare to ordinary public access; "
    ...
    def scrape(self) -> List[ArrestRecord]:
        ...
        try:
            ...
            if resp.status_code in (403, 503):
                logger.warning(
                    f"Hampton: HTTP {resp.status_code} — bot protection. "
                    f"Set SOCKS_PROXY for residential IP."
                )
                return []
            resp.raise_for_status()
            records = self._parse(resp.text)
            logger.info(f"Hampton: {len(records)} records in {time.time()-start:.1f}s")
            return records
        except Exception as e:
            logger.error(f"Hampton scrape failed: {e}")
            return []
    ...
    def _parse(self, html: str) -> List[ArrestRecord]:
        ...
        for table in soup.find_all("table"):
            ...
            for tr in rows[1:]:
                ...
                booking = cells[2] if len(cells) > 2 else f"SC_{hashlib.md5(f'{name}|HAMPTO'.encode()).hexdigest()[:10]}"
