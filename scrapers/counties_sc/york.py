        ...
        try:
            ...
            try:
                ...
                if isinstance(bookings, list) and bookings and isinstance(bookings[0], dict):
                    ...
                    logger.info(f"York: {len(records)} JSON records in {time.time()-start:.1f}s")
                    return records
            except Exception:
                pass
            ...
            for row in table.find_all("tr")[1:]:
                ...
                booking_num = f"SC_{hashlib.md5(f'{full_name}|{booking_date_str}|YORK'.encode()).hexdigest()[:10]}"
                records.append(ArrestRecord(
                    County=self.county,
                    State="SC",
