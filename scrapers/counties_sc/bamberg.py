...
Portal: https://bambergjailroster.org/inmate-search/
Often blocked (403/Cloudflare) from datacenter IPs. Attempts HTTP fetch;
set BAMBERG_SOCKS_PROXY or SOCKS_PROXY for residential egress.
"""
    ...
    def scrape(self) -> List[ArrestRecord]:
        ...
        try:
            ...
            if resp.status_code in (403, 503):
                logger.warning(
                    f"Bamberg: HTTP {resp.status_code} — bot protection. "
                    f"Set SOCKS_PROXY for residential IP."
                )
                return []
            resp.raise_for_status()
            records = self._parse(resp.text)
            logger.info(f"Bamberg: {len(records)} records in {time.time()-start:.1f}s")
            return records
        except Exception as e:
            logger.error(f"Bamberg scrape failed: {e}")
            return []
