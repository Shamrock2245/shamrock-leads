"""
Midland County (TX) Arrest Scraper — Midland Sheriff Roster.
URL: https://www.co.midland.tx.us/sheriff/inmates
"""
import logging
import time
...
class MidlandScraper(BaseScraper):
    ...
    def scrape(self) -> List[ArrestRecord]:
        ...
        url = "https://www.co.midland.tx.us/api/jail/roster"
        headers = {
                ...
                for item in resp.json().get("inmates", []):
                    ...
                    records.append(
                        ArrestRecord(
                            ...
                            facility="Midland County Detention Center",
                            scraped_at=time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                        )
                    )
        except Exception as e:
            logger.info(f"Midland TX scrape info: {e}")
        return records
