"""
Hays County (TX) Arrest Scraper — San Marcos / Hays Sheriff Roster.
URL: https://hayscountytx.com/sheriff/inmates
"""
import logging
import time
...
class HaysScraper(BaseScraper):
    ...
    def scrape(self) -> List[ArrestRecord]:
        ...
        url = "https://hayscountytx.com/api/v1/inmates"
        headers = {
                ...
                for item in resp.json().get("inmates", []):
                    ...
                    records.append(
                        ArrestRecord(
                            ...
                            facility="Hays County Law Enforcement Center",
                            scraped_at=time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                        )
                    )
        except Exception as e:
            logger.info(f"Hays TX scrape info: {e}")
        return records
