"""
Hidalgo County (TX) Arrest Scraper.

Portal: https://www.hidalgocounty.us/sheriff or Hidalgo Jail Roster
Hidalgo County is a major South Texas border county (~880k pop).

Dedup key: Booking_Number (Booking ID / SO Number)
"""
...
PORTAL_URL = "https://www.hidalgocounty.us/sheriff"
SEARCH_URL = "https://www.hidalgocounty.us/sheriff/inmatesearch.php"

LETTER_WALK = ["A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "L", "M", "N", "P", "R", "S", "T", "W"]


class HidalgoScraper(BaseScraper):
    @property
    def county(self) -> str:
        return "Hidalgo"
    ...
    def scrape(self) -> List[ArrestRecord]:
        ...
        for letter in letters:
            try:
                ...
                logger.warning(f"Hidalgo letter {letter} failed: {e}")

        logger.info(f"✅ Hidalgo (TX): {len(records)} records in {time.time() - start:.1f}s")
        return records

    def _scrape_letter(self, letter: str, seen: set) -> List[ArrestRecord]:
        params = {"lname": letter, "fname": ""}
        ...
        for row in rows[1:]:
            ...
            out.append(
                ArrestRecord(
                    ...
                    Status="In Custody",
                    Facility="Hidalgo County Adult Detention Center",
