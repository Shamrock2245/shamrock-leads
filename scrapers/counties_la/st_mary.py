"""St. Mary Parish, Louisiana public jail-roster scraper.

Source: https://www.stmaryso.com/inmate-roster/filters/current/booking_time=desc/1

The St. Mary Parish Sheriff's Office publishes a broad public roster with a
source-issued Booking # and Booking Date. The scraper uses those public fields
directly and does not fetch individual profile pages.
"""
...
class StMaryLARosterScraper(BaseScraper):
    """Scrape St. Mary Parish Sheriff's Office's public current roster."""

    PORTAL_URL = "https://www.stmaryso.com/inmate-roster/filters/current/booking_time=desc/1"
    MAX_PAGES = 100
    ...
    def _card_to_record(self, card) -> ArrestRecord | None:
        ...
        return ArrestRecord(
            ...
            Status="In Custody",
            Facility="St. Mary Parish Jail",
            Agency="St. Mary Parish Sheriff",
