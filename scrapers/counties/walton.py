"""
Walton County Arrest Scraper — New World InmateInquiry
...
Fields: Name, Subject Number, In Custody, Scheduled Release Date, Gender, DOB, Height, Weight
"""
...
API_URL = "https://nwscorrections.waltonso.org/NewWorld.InMateInquiry/WaltonCounty/api/Inmates"
...
class WaltonCountyScraper(BaseScraper):
    ...
    def scrape(self) -> List[ArrestRecord]:
            ...
            if resp.status_code == 200:
                try:
                    ...
                    if inmates:
                        ...
                        logger.info(f"Walton JSON API: {len(records)} records")
                        return records
                except Exception:
                    pass
        except Exception as e:
            logger.debug(f"Walton JSON API failed: {e}")
        ...
        try:
            ...
            logger.info(f"Walton HTML: {len(records)} records")
            return records
        except Exception as e:
            logger.error(f"Walton HTML failed: {e}")
            return []
    ...
    def _parse_json(self, inmates: list) -> List[ArrestRecord]:
        ...
        for inmate in inmates:
            try:
                ...
                logger.debug(f"Walton parse error: {e}")
        return records
