...
|-------|--------------------:|-----------|-------|
| **GA** | **85** | `scrapers/counties_ga/` | Gwinnett and Fulton fail-closed guards are deployed; six audited legacy P2C paths also fail closed under `0de5f79`. Five JailTracker wrappers are locally hardened fail closed pending deployment; see `docs/LEGACY_P2C_SOURCE_SAFETY.md` and `docs/JAILTRACKER_SOURCE_SAFETY.md`. |
| **FL** | **67** | `scrapers/counties/` | Miami-Dade ArcGIS repair and Broward fail-closed guard deployed 2026-08-14. Seven JailTracker wrappers are locally hardened fail closed pending deployment; see `docs/JAILTRACKER_SOURCE_SAFETY.md`. Public hosts are healthy and per-source production telemetry remains pending. |
...
| **SC** | **46** | `scrapers/counties_sc/` | York source-faithful parser repair is deployed; Lee and Lexington legacy P2C paths fail closed under `0de5f79`. Anderson, Cherokee, Colleton, Kershaw, and Laurens Zuercher guards are deployed in `7718bf8`; Chester and Greenwood JailTracker wrappers are locally hardened fail closed pending deployment. See `docs/SC_ZUERCHER_SOURCE_SAFETY.md` and `docs/JAILTRACKER_SOURCE_SAFETY.md`. Per-county persistence and alert telemetry remain pending. |
...
| **TN** | **22** | `scrapers/counties_tn/` | + Putnam (deployed 2026-08-12 EDT; public ISOMS source locally validated; Mongo upsert/alert telemetry still pending). Five JailTracker wrappers are locally hardened fail closed pending deployment; see `docs/JAILTRACKER_SOURCE_SAFETY.md`. |
| **AL** | **16** | `scrapers/counties_al/` | Lee, Marshall, St. Clair, and Etowah are deployed after bounded official-roster smokes; Madison, Mobile, and Montgomery safeguards are deployed fail closed. Shelby is locally hardened under the shared JailTracker guard pending deployment. Public hosts are healthy. Per-scraper Mongo/alert evidence remains pending. |
...
| **MS** | **9** | `scrapers/counties_ms/` | Registry reconciled 2026-08-14; 9 jobs registered. Adams, Lafayette, Lowndes, Oktibbeha, and Warren remain recon-only; four JailTracker wrappers are locally hardened fail closed pending deployment. See `docs/JAILTRACKER_SOURCE_SAFETY.md`. |
