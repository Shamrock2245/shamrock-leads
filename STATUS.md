...
| **AL** | **16** | `scrapers/counties_al/` | Lee, Marshall, St. Clair, and Etowah are deployed after bounded official-roster smokes; Madison and Mobile are deployed fail closed, while Montgomery is locally fail closed after its official public API returned 403 through normal access. Public hosts are healthy. Per-scraper Mongo/alert evidence remains pending. |
