...
| **FL** | **67** | `scrapers/counties/` | Miami-Dade ArcGIS repair and Broward fail-closed guard deployed 2026-08-14. Sarasota’s third-party/proxy/CAPTCHA override is locally validated and pending deployment; seven inherited JailTracker wrappers are deployed fail closed under `0a75169`. See `docs/JAILTRACKER_SOURCE_SAFETY.md`. Public hosts are healthy and per-source production telemetry remains pending. |
...
| **AL** | **16** | `scrapers/counties_al/` | Lee, Marshall, St. Clair, and Etowah are deployed after bounded official-roster smokes; Madison, Mobile, and Montgomery safeguards are deployed fail closed. Shelby is deployed fail closed under `0a75169`. Public hosts are healthy. Per-scraper Mongo/alert evidence remains pending. |
