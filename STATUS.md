...
| **AL** | **16** | `scrapers/counties_al/` | Lee, Marshall, St. Clair, and Etowah are deployed after bounded official-roster smokes; Madison, Mobile, and Montgomery safeguards are deployed fail closed. Shelby is deployed fail closed under `0a75169`. Tuscaloosa’s wrong-jurisdiction path is locally replaced with a fail-closed guard after the official ‘Who’s in Jail’ route required human verification; deployment is pending. Public hosts are healthy. Per-scraper Mongo/alert evidence remains pending. |
