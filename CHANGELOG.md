...
### Changed
- **Gwinnett County, GA source-safety repair** — removed the unsafe empty SmartWEB form submission and record construction that omitted source booking numbers and complete identities. The official public last-24-hours view abbreviates given names, so `scraper_ga_gwinnett` now fails closed with zero emitted records until a supported bulk identity contract is available. Added deterministic no-network regression tests.
