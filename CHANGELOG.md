...
- **Durham County, NC source-safety repair** — replaced the stale legacy ASP.NET A–Z broad-search path, which disabled TLS verification and could emit records without a booking-date boundary, with a fail-closed guard. `scraper_nc_durham` remains registered but emits no records until a supported public bulk roster exposes complete identity plus source-issued booking fields. Added deterministic no-network regression tests.
