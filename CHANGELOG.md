...
- **Mobile County, AL source-safety repair** — removed residential-proxy access, DOB retention, and synthetic `MOB_*` booking IDs from the registered path. The official current-inmates portal has not established a supported public booking-safe broad roster through normal access, so `scraper_al_mobile` now fails closed without making a network request. Added deterministic no-network regression tests. Production persistence and alert delivery remain unproven pending deployment.
