...
- **Madison County, AL source-safety repair** — removed residential-proxy discovery, broad endpoint harvesting, and synthetic `MAD_*` booking IDs from the registered path. The official inmate-information surface has not established a supported public booking-safe broad roster through normal access, so `scraper_al_madison` now fails closed without making a network request. Added deterministic no-network regression tests. Production persistence and alert delivery remain unproven pending deployment.
