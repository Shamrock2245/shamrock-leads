...
- **Tuscaloosa County, AL source-safety repair** — removed a legacy path that targeted Tulsa County, Oklahoma, fell back to an unsourced `id`, and constructed records with non-canonical schema fields. Tuscaloosa’s official Sheriff ‘Who’s in Jail’ route requires human verification and does not establish a broad booking-safe public roster through normal access. `scraper_al_tuscaloosa` now fails closed without making a network request. Added deterministic no-network regression tests. Deployment, persistence, and alert delivery remain unproven pending rollout.
