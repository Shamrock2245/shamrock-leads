...
from scrapers.counties_tn.coffee import CoffeeTNScraper


class CoffeeSafetyTests(unittest.TestCase):
    def test_fails_closed_without_network_or_records(self):
        scraper = CoffeeTNScraper()
    ...
    def test_no_generic_parser_invocation(self):
        source = Path('scrapers/counties_tn/coffee.py').read_text(encoding='utf-8')
