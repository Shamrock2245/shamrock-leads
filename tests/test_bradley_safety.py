...
from scrapers.counties_tn.bradley import BradleyScraper


class BradleySafetyTests(unittest.TestCase):
    def test_fails_closed_without_network_or_records(self):
        scraper = BradleyScraper()
    ...
    def test_no_generic_parser_invocation(self):
        source = Path('scrapers/counties_tn/bradley.py').read_text(encoding='utf-8')
