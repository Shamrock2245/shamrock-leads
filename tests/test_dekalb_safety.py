...
from scrapers.counties_al.dekalb import DeKalbALScraper


class DeKalbSafetyTests(unittest.TestCase):
    def test_fails_closed_without_network_or_records(self):
        scraper = DeKalbALScraper()
    ...
    def test_no_generic_parser_invocation(self):
        source = Path('scrapers/counties_al/dekalb.py').read_text(encoding='utf-8')
