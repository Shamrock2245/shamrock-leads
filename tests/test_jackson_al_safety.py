...
from scrapers.counties_al.jackson import JacksonALScraper


class JacksonALSafetyTests(unittest.TestCase):
    def test_fails_closed_without_network_or_records(self):
        scraper = JacksonALScraper()
    ...
    def test_no_generic_parser_invocation(self):
        source = Path('scrapers/counties_al/jackson.py').read_text(encoding='utf-8')
