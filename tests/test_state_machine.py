...
# ── Helpers ───────────────────────────────────────────────────────────────────
def _make_db(bond_doc=None, packet_doc=None):
    ...
    paperwork_packets = MagicMock()
    # Default: no packet (allows retrospective manual bond entry to active)
    paperwork_packets.find_one = AsyncMock(return_value=packet_doc)

    db.active_bonds = active_bonds
    db.paperwork_packets = paperwork_packets
    return db
...
class TestBondStateMachine:
    ...
    @patch("dashboard.routers.events.publish_event", new_callable=AsyncMock)
    async def test_valid_transition_active_to_exonerated(
        self, mock_publish, mock_auto_release, mock_schedule, mock_cancel, mock_audit, mock_get_db
    ):
    ...
    @patch("dashboard.services.task_engine.TaskEngine.schedule_compliance_tasks", new_callable=AsyncMock)
    @patch("dashboard.routers.events.publish_event", new_callable=AsyncMock)
    async def test_valid_transition_monitoring_to_active(
        self, mock_publish, mock_schedule, mock_cancel, mock_audit, mock_get_db
    ):
        # Packet must be signed/completed for transition into active (or absent)
        mock_db = _make_db(
            {"booking_number": "123", "status": "monitoring"},
            packet_doc={"status": "signed", "booking_number": "123"},
        )
        mock_get_db.return_value = mock_db
    ...
    @patch("dashboard.services.state_machine.get_db")
    async def test_active_blocked_when_packet_unsigned(self, mock_get_db):
        mock_db = _make_db(
            {"booking_number": "123", "status": "monitoring"},
            packet_doc={"status": "sent", "booking_number": "123"},
        )
        mock_get_db.return_value = mock_db

        with pytest.raises(ValueError, match="Document Group is not signed"):
            await BondStateMachine.transition_bond("123", "active", "TestAgent")
