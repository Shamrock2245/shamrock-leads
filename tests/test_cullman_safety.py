...
from scrapers.counties_al.cullman import CullmanScraper


class CullmanSafetyTests(unittest.TestCase):
    def test_fails_closed_without_network_or_records(self):
        scraper = CullmanScraper()
    ...
    def test_no_generic_parser_invocation(self):
        source = Path('scrapers/counties_al/cullman.py').read_text(encoding='utf-8')
