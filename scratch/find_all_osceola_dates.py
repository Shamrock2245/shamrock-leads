...
# Group Osceola records by scraped_at / scrape_timestamp
pipeline = [
    {"$match": {"county": "Osceola"}},
    {"$group": {
        "_id": "$scrape_timestamp",
        "count": {"$sum": 1},
...
print("Osceola batches in database:")
for r in results[:30]:
    print(f"Timestamp: {r['_id']}, Total Records: {r['count']}, Shifted Records: {r['has_shifted']}")
