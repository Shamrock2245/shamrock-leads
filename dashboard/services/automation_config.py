...
DEFAULT_CONFIG = {
    ...
    "first_appearance_watcher": {
        ...
        "interval_seconds": 1800,       # 30 minutes
        "slack_digest": True,
        "target_counties": fa_watch_counties(),
    },
...
async def get_automation_config(db) -> dict:
    ...
    # Union write-eligible and watch counties into first_appearance_watcher.target_counties
    faw = cfg.get("first_appearance_watcher")
    if isinstance(faw, dict):
        stored_targets = faw.get("target_counties")
        if isinstance(stored_targets, list):
            merged_targets = list(stored_targets)
            changed = False
            for c in fa_watch_counties():
                if not any(c.casefold() == str(st).casefold() for st in merged_targets):
                    merged_targets.append(c)
                    changed = True
            if changed:
                faw["target_counties"] = merged_targets
                cfg["first_appearance_watcher"] = faw
                await config_coll.update_one(
                    {"type": "automation_master"},
                    {"$set": {"first_appearance_watcher.target_counties": merged_targets}},
                )
                logger.info("☘️  FA Watcher target counties unioned write-eligible counties: %s", merged_targets)
