"""
ShamrockLeads — SignNow Service (Phase E — Manus)
Async SignNow API client: copy template, prefill fields, embedded invites,
download PDF, conflict handler.
"""
...
class SignNowService:
    ...
    async def create_embedded_invite(self, document_id: str, signers: List[Dict[str, Any]], name_formula: str = "") -> dict:
        ...
        async with httpx.AsyncClient() as client:
            payload = {"invites": signers, "name_formula": name_formula}
            resp = await client.post(
    ...
    async def get_signing_link(self, link_id: str, expiration: int = 45) -> str:
        ...
        async with httpx.AsyncClient() as client:
            payload = {"auth_method": "none", "link_expiration": expiration}
            resp = await client.post(
    ...
    async def create_full_packet(self, defendant_data: dict, surety_id: str) -> dict:
        ...
        # Placeholder — will use template IDs from config.py
        pass
