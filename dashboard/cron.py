...
async def _run_swipesimple_gmail_poll():
    ...
    db = get_db()
    cfg = (await get_automation_config(db)).get("swipesimple_gmail_poll") or {}
    if cfg.get("enabled") is False:
        return
    result = await run_swipesimple_receipt_poll(cfg)
    await db["automation_run_log"].insert_one({
        "automation": "swipesimple_gmail_poll",
        "run_at": datetime.now(timezone.utc),
        "result": result,
    })
    if result.get("processed"):
        logger.info(
            "[SwipeSimpleGmail] processed=%s school_skip=%s errors=%s",
            result.get("processed"),
            result.get("skipped_school"),
            result.get("errors"),
        )
