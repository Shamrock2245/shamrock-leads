...
geo_intel_bp = Blueprint("geo_intelligence", __name__, url_prefix="/api/geo-intel")
traccar_webhook_bp = Blueprint("traccar_webhook", __name__, url_prefix="/api/traccar")
...
@geo_intel_bp.route("/health", methods=["GET"])
...
@geo_intel_bp.route("/devices", methods=["GET"])
async def list_devices():
    """List all registered tracking devices, optionally by booking number."""
    booking = request.args.get("booking_number")
    svc = _get_service()
    devices = await svc.list_devices(booking)
    return jsonify({"devices": devices, "count": len(devices)})


@geo_intel_bp.route("/devices", methods=["POST"])
async def register_device():
    ...
    if not booking or not unique_id:
        return jsonify({"error": "booking_number and unique_id are required"}), 400
    ...
    try:
        ...
        return jsonify({"error": f"Traccar error: {str(e)}"}), 502
    ...
    return jsonify({"device": device, "traccar_id": traccar_id}), 201


@geo_intel_bp.route("/devices/<device_id>/deactivate", methods=["POST"])
...
@geo_intel_bp.route("/positions/latest", methods=["GET"])
...
@geo_intel_bp.route("/positions/route", methods=["GET"])
async def device_route():
    """Get full position trail for a device.

    Query: ?traccar_device_id=&from=&to= (ISO 8601)
    """
    device_id = request.args.get("traccar_device_id", type=int)
    from_dt = request.args.get("from", "")
    to_dt = request.args.get("to", "")
    if not device_id or not from_dt or not to_dt:
        return jsonify({"error": "traccar_device_id, from, and to are required"}), 400

    traccar = _get_traccar()
    try:
        route = await traccar.get_route(device_id, from_dt, to_dt)
        return jsonify({"route": route, "count": len(route)})
    except Exception as e:
        return jsonify({"error": str(e)}), 502
...
@geo_intel_bp.route("/zones", methods=["GET"])
...
@geo_intel_bp.route("/zones", methods=["POST"])
async def create_zone():
    ...
    if data["zone_type"] not in ("inclusion", "exclusion"):
        return jsonify({"error": "zone_type must be 'inclusion' or 'exclusion'"}), 400
    ...
    return jsonify({"zone": zone, "traccar_geofence_id": traccar_fence_id}), 201


@geo_intel_bp.route("/zones/<zone_id>", methods=["DELETE"])
...
@geo_intel_bp.route("/vehicle-watch", methods=["GET"])
async def list_vehicle_watches():
    """List all active vehicle watches."""
    svc = _get_service()
    watches = await svc.list_vehicle_watches()
    return jsonify({"vehicles": watches, "count": len(watches)})


@geo_intel_bp.route("/vehicle-watch", methods=["POST"])
async def add_vehicle_watch():
    ...
    if not data.get("booking_number") or not data.get("vehicle_info"):
        return jsonify({"error": "booking_number and vehicle_info required"}), 400
    ...
    return jsonify({"vehicle_watch": watch}), 201


@geo_intel_bp.route("/vehicle-watch/<watch_id>/sighting", methods=["POST"])
async def record_vehicle_sighting(watch_id: str):
    ...
    if not ok:
        return jsonify({"error": "Vehicle watch not found"}), 404
...
@geo_intel_bp.route("/violations", methods=["GET"])
async def violation_feed():
    ...
    return jsonify({"violations": events, "count": len(events)})


@geo_intel_bp.route("/violations/<event_id>/acknowledge", methods=["POST"])
...
@geo_intel_bp.route("/overview", methods=["GET"])
...
@geo_intel_bp.route("/photo-checkin", methods=["POST"])
async def photo_checkin():
    ...
    if not data.get("booking_number") or not data.get("photo_url"):
        return jsonify({"error": "booking_number and photo_url required"}), 400
    ...
    return jsonify({"checkin": checkin}), 201
...
@traccar_webhook_bp.route("/webhook", methods=["POST"])
async def traccar_webhook():
    ...
    if not position:
        ...
        return jsonify({"ok": True, "type": "event_only"})
    ...
    if result:
        ...
        return jsonify({"ok": True, "synced": False, "reason": "No matching device binding"})
