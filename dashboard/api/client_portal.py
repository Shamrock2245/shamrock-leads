...
portal_bp = Blueprint("client_portal", __name__)
...
@portal_bp.route("/c/<token>", methods=["GET"])
...
@portal_bp.route("/api/portal/<token>/status", methods=["GET"])
...
@portal_bp.route("/api/portal/<token>/checkin", methods=["POST"])
async def portal_checkin(token: str):
    ...
    if token_data["role"] != "defendant":
        return jsonify({"error": "Check-in is only available for defendants"}), 403
...
@portal_bp.route("/api/portal/<token>/payment-link", methods=["GET"])
...
@portal_bp.route("/api/portal/generate", methods=["POST"])
async def generate_token_endpoint():
    ...
    if role not in ("defendant", "indemnitor"):
        return jsonify({"error": "role must be 'defendant' or 'indemnitor'"}), 400
...
@portal_bp.route("/api/portal/tokens/<booking_number>", methods=["GET"])
...
@portal_bp.route("/api/portal/revoke", methods=["POST"])
...
@portal_bp.route("/api/portal/send-link", methods=["POST"])
async def send_portal_link():
    ...
    if not booking_number or not phone:
        return jsonify({"error": "booking_number and phone are required"}), 400
