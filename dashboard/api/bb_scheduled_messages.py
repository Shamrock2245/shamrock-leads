...
bb_schedule_bp = Blueprint("bb_scheduled_messages", __name__)
...
@bb_schedule_bp.route("/bb-schedule/court-reminders", methods=["POST"])
...
@bb_schedule_bp.route("/bb-schedule/document-ready", methods=["POST"])
async def api_schedule_document_ready():
    ...
    try:
        ...
        if not phone or not defendant_name or not signing_url:
            return jsonify({"success": False, "error": "phone, defendant_name, signing_url required"}), 400
...
@bb_schedule_bp.route("/bb-schedule/pending", methods=["GET"])
...
@bb_schedule_bp.route("/bb-schedule/process", methods=["POST"])
