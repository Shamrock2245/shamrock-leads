...
rearrest_bp = Blueprint("rearrest_notifier", __name__)
...
@rearrest_bp.route("/rearrest/check", methods=["POST"])
...
@rearrest_bp.route("/rearrest/notify", methods=["POST"])
async def api_rearrest_notify():
    ...
    try:
        ...
        if not phone or not defendant_name:
            return jsonify({"success": False, "error": "phone and defendant_name required"}), 400
...
@rearrest_bp.route("/rearrest/history", methods=["GET"])
async def api_rearrest_history():
    ...
    """
    try:
        defendant_name = request.args.get("defendant_name", "")
...
@rearrest_bp.route("/rearrest/stats", methods=["GET"])
