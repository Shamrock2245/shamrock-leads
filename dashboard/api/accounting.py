...
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/dashboard")
...
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/transactions")
async def api_accounting_transactions():
    ...
    surety = request.args.get("surety", "")
    unattributed = request.args.get("unattributed", "").lower() == "true"
    ...
    return jsonify({"transactions": docs, "total": total, "page": page, "limit": limit})


# ═══════════════════════════════════════════════════════════════════════════════
#  POST /api/accounting/transactions — Record a transaction (cash, check, etc.)
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/transactions", methods=["POST"])
async def api_record_transaction():
    ...
    return jsonify({"success": True, "transaction": txn}), 201


# ═══════════════════════════════════════════════════════════════════════════════
#  PATCH /api/accounting/transactions/<txn_id>/attribute — Link txn to case
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/transactions/<txn_id>/attribute", methods=["PATCH"])
...
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/import/swipesimple", methods=["POST"])
async def api_import_swipesimple():
    ...
    return jsonify({"success": True, "batch_id": batch_id, "imported": imported, "skipped": skipped, "errors": len(errors_list)})


# ═══════════════════════════════════════════════════════════════════════════════
#  GET /api/accounting/import/history — Import batch history
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/import/history")
async def api_import_history():
    ...
    async for doc in imports.find().sort("timestamp", -1).limit(20):
        doc["_id"] = str(doc["_id"])
        docs.append(doc)
    return jsonify({"imports": docs})


# ═══════════════════════════════════════════════════════════════════════════════
#  GET /api/accounting/premium-split — Calculate splits for a bond
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/premium-split")
async def api_premium_split():
    bond_amount = float(request.args.get("bond_amount", 0))
    surety = request.args.get("surety", "osi").lower()
    if bond_amount <= 0:
        return jsonify({"error": "bond_amount required"}), 400
...
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/revenue/monthly")
async def api_revenue_monthly():
    ...
    return jsonify({"monthly": [{"month": r["_id"], "total": r["total"], "count": r["count"]} for r in results]})


# ═══════════════════════════════════════════════════════════════════════════════
#  GET /api/accounting/export/quickbooks — QuickBooks-compatible CSV export
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/export/quickbooks")
...
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/export/csv")
...
# ═══════════════════════════════════════════════════════════════════════════════
@accounting_bp.route("/accounting/transactions/<txn_id>", methods=["DELETE"])
