...
bb_contacts_bp = Blueprint("bb_contact_sync", __name__)
...
@bb_contacts_bp.route("/bb-contacts/sync-bond", methods=["POST"])
async def api_sync_bond_contacts():
    ...
    try:
        ...
        if not bond:
            return jsonify({"success": False, "error": f"Bond {booking_number} not found"}), 404

        result = await sync_bond_contacts(bond)
        return jsonify({"success": True, "booking_number": booking_number, **result})
...
@bb_contacts_bp.route("/bb-contacts/sync-all", methods=["POST"])
...
@bb_contacts_bp.route("/bb-contacts/check", methods=["POST"])
async def api_check_imessage():
    ...
    try:
        ...
        if not phones:
            return jsonify({"success": False, "error": "phones array required"}), 400
...
@bb_contacts_bp.route("/bb-contacts/list", methods=["GET"])
