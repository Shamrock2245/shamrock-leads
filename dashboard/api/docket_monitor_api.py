...
docket_monitor_bp = Blueprint("docket_monitor", __name__, url_prefix="/api/docket-monitor")
...
@docket_monitor_bp.route("/status", methods=["GET"])
...
@docket_monitor_bp.route("/events", methods=["GET"])
...
@docket_monitor_bp.route("/events/<event_id>", methods=["GET"])
...
@docket_monitor_bp.route("/bond/<bond_case_id>", methods=["GET"])
...
@docket_monitor_bp.route("/alerts", methods=["GET"])
...
@docket_monitor_bp.route("/scan", methods=["POST"])
...
@docket_monitor_bp.route("/acknowledge", methods=["POST"])
...
@docket_monitor_bp.route("/acknowledge-all", methods=["POST"])
async def acknowledge_all():
    ...
    try:
        ...
        return jsonify({"success": True, "acknowledged_count": result.modified_count})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500


@docket_monitor_bp.route("/risk-timeline/<bond_case_id>", methods=["GET"])
async def risk_timeline(bond_case_id):
    ...
    try:
        ...
        return jsonify({"success": True, "bond_case_id": bond_case_id,
