...
@payment_plans_bp.route('/payments/premium-calc', methods=['GET'])
async def premium_calculator():
    ...
    surety_id = request.args.get('surety_id', 'osi').lower()
    if bond_amount <= 0:
        return jsonify({"error": "bond_amount must be positive"}), 400
    return jsonify(calculate_premium(bond_amount, surety_id))


# ── Payment Plans CRUD ──────────────────────────────────────────────────────────

@payment_plans_bp.route('/payments/plans', methods=['GET'])
async def list_plans():
    ...
    return jsonify({"plans": results, "total": len(results)})


@payment_plans_bp.route('/payments/plans/<booking_number>', methods=['GET'])
async def get_plan(booking_number):
    ...
    if not doc:
        return jsonify({"error": "No payment plan found"}), 404
    doc["_id"] = str(doc["_id"])
...
@payment_plans_bp.route('/payments/plans', methods=['POST'])
async def create_plan():
    ...
    if existing:
        return jsonify({"error": "Active payment plan already exists for this booking"}), 409
    ...
    return jsonify({"success": True, "plan": plan_doc}), 201


@payment_plans_bp.route('/payments/plans/<plan_id>/pay', methods=['POST'])
...
async def get_delinquent():
    ...
    return jsonify({"delinquent": delinquent, "total": len(delinquent), "threshold_days": threshold_days})


# ── Revenue Summary ─────────────────────────────────────────────────────────────

@payment_plans_bp.route('/payments/summary', methods=['GET'])
