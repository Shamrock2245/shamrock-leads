  ...
  try {
    ...
    grid.innerHTML = (d.leads||[]).map(l => {
      ...
      return `<div class="def-card">
        ...
        <div class="def-card-footer"><button class="btn-detail" onclick="window.open('${l.detail_url||'#'}')">🔗 Source</button><button class="btn-write-bond" onclick="SL.openBondModal('${(l.full_name||'').replace(/'/g,"\\'")}',${bond},'${l.county||''}','${l.booking_number||''}')">✍️ Write Bond</button></div>
      </div>`;
