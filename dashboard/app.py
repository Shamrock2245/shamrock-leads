...
def api_indemnitors_list():
    ...
    Supports both legacy single `indemnitor` and new `indemnitors[]` array."""
    try:
        search = request.args.get("search", "").strip()
        ...
        def _extract_indemnitors(doc, bond_type, stage=""):
            """Extract indemnitor(s) from a bond document, handling both formats."""
            items = []
            indemnitors = doc.get("indemnitors", [])
            if not indemnitors and doc.get("indemnitor"):
                # Legacy single indemnitor
        ...
        p_query = {"$or": [{"indemnitor": {"$exists": True}}, {"indemnitors": {"$exists": True}}]}
        if search:
        ...
        a_query = {"$or": [{"indemnitor": {"$exists": True}}, {"indemnitors": {"$exists": True}}]}
        if search:
...
# ── Indemnitor Document/Image Upload (KYC) ──────────────────────────────────────
...
@app.route("/api/indemnitors/<booking_number>/uploads", methods=["POST"])
def api_indemnitor_upload(booking_number):
    ...
    try:
        ...
        if ext not in ALLOWED_UPLOAD_EXTENSIONS:
            return jsonify({"error": f"File type .{ext} not allowed. Use: {', '.join(ALLOWED_UPLOAD_EXTENSIONS)}"}), 400
        ...
        # Update whichever collection has this booking
...
@app.route("/api/indemnitors/<booking_number>/uploads", methods=["GET"])
def api_indemnitor_uploads_list(booking_number):
...
@app.route("/api/indemnitors/<booking_number>/uploads/<file_id>", methods=["DELETE"])
def api_indemnitor_upload_delete(booking_number, file_id):
    """Delete a specific uploaded KYC document."""
    try:
        for coll in [prospective_bonds, active_bonds]:
            doc = coll.find_one({"booking_number": booking_number})
            if doc:
                uploads = doc.get("kyc_uploads", [])
...
@app.route("/uploads/<booking_number>/<filename>")
def serve_upload(booking_number, filename):
    """Serve uploaded files (for preview in dashboard)."""
...
# ── Cross-Entity Search ──
@app.route("/api/indemnitors/search-existing", methods=["GET"])
def api_indemnitor_search_existing():
    """Smart search across arrests, prospective_bonds, active_bonds, and indemnitors.
    ...
    try:
        ...
        results = []
        seen_keys = set()   # phone-based dedup
        ...
        for doc in db.arrests.find(arrest_query).limit(20):
            phone = doc.get("phone", "")
        ...
        for doc in prospective_bonds.find(pb_query).limit(20):
            ind = doc.get("indemnitor", {})
            ind_name = ind.get("name") or " ".join(filter(None, [ind.get("firstName", ""), ind.get("lastName", "")])) or ""
            phone = ind.get("phone", "")
        ...
        for doc in active_bonds.find(ab_query).limit(20):
            ind = doc.get("indemnitor", {})
...
@app.route("/api/indemnitors/create", methods=["POST"])
def api_indemnitor_create():
    """Create or update an indemnitor profile and optionally link to a bond.
    Deduplicates by phone number — if phone exists, updates existing record."""
    try:
        data = request.get_json(force=True)
        now = datetime.now(timezone.utc)
        ...
        if not booking_number:
            return jsonify({"error": "booking_number required to link indemnitor to a bond"}), 400
        ...
        if not existing_indemnitors and doc.get("indemnitor"):
            # Migrate legacy single indemnitor
            old_ind = doc.get("indemnitor", {})
            old_name = old_ind.get("name") or " ".join(filter(None, [old_ind.get("firstName", ""), old_ind.get("lastName", "")])) or ""
        ...
        # Update bond: keep legacy `indemnitor` field pointing to primary
...
@app.route("/api/indemnitors/<booking_number>/remove", methods=["POST"])
def api_indemnitor_remove(booking_number):
    ...
    try:
        ...
        if len(existing) <= 1:
            return jsonify({"error": "Cannot remove the last indemnitor. Each bond must have at least one."}), 400
        ...
        return jsonify({"success": True, "indemnitors": updated})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
# REMOVED: Duplicate sync routes that conflicted with async Quart blueprints.
# These endpoints are now handled exclusively by:
#   - dashboard/api/rearrest_notifier.py → /api/rearrest/pending, /dismiss, /contacted
#   - dashboard/api/rearrest_detector.py → /api/rearrest/scan, /alerts
# Both blueprints are registered in dashboard/__init__.py with url_prefix="/api".
