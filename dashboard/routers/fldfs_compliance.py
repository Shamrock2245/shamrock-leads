...
fldfs_bp = APIRouter(prefix="/api", tags=["fldfs_compliance"])
...
async def full_filing(request: Request):
    ...
    try:
        # Direct async calls — no Quart test_request_context needed in FastAPI
        summary   = await monthly_summary(request)
        agent_data = await agent_1099(request)
        poa_data   = await poa_utilization(request)
        forf_data  = await forfeiture_log(request)
