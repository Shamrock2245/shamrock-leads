...
async def log_payment(request: Request):
    ...
    return JSONResponse(
        status_code=201,
        content={"success": True, "payment_id": payment_doc['_id']},
    )


@payments_bp.get("/payments/{booking_number}")
async def get_payments(booking_number: str):
...
async def create_payment_plan(request: Request, booking_number: str):
    ...
    return JSONResponse(
        status_code=201,
        content={"success": True, "plan_id": plan_doc['_id']},
    )
...
async def add_ledger_entry(request: Request):
    ...
    data = await request.json()
    if not data or 'booking_number' not in data or 'amount' not in data \
            or 'type' not in data or 'category' not in data:
        return JSONResponse({"error": "Missing required fields"}, status_code=400)
    ...
    try:
        ...
        return JSONResponse(
            status_code=201,
            content={"success": True, "transaction_id": transaction_id},
        )
...
async def import_swipesimple(request: Request):
    """Upload a SwipeSimple CSV file and parse entries into the ledger."""
    from dashboard.services.ledger_service import LedgerService
