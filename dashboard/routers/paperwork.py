...
async def generate_swipesimple_link(request: Request):
    ...
    Premium is written into SMS/email copy; checkout URL is the static
    SWIPESIMPLE_PAYMENT_LINK (SwipeSimple plan does not amount-bind the URL).
    """
    try:
        body = await request.json() if request else {}
        from dashboard.services.packet_payment_link_service import (
            send_swipesimple_payment_link,
        )

        deliver_master = body.get("deliver", True)
        result = await send_swipesimple_payment_link(
            packet_id=(body.get("packet_id") or "").strip(),
            booking_number=(body.get("booking_number") or "").strip(),
            amount=body.get("amount"),
            phone=(body.get("phone") or "").strip(),
            email=(body.get("email") or body.get("email_address") or "").strip(),
            defendant_name=(body.get("defendant_name") or "").strip(),
            deliver=bool(deliver_master),
        ...
        if not result.get("success") and result.get("error"):
            return JSONResponse(
                {"success": False, "error": result.get("error"), **{k: v for k, v in result.items() if k != "error"}},
                status_code=500,
            )
        return result
    except Exception as exc:
